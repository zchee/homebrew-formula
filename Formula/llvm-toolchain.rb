class LlvmToolchain < Formula
  ...
  def install
    ...
    args = %W[
      ...
      -DCMAKE_OSX_ARCHITECTURES=#{shell_output("uname -m").chomp}
      -DCMAKE_OSX_DEPLOYMENT_TARGET=#{shell_output("xcrun --sdk macosx --show-sdk-version").chomp}
      -DCMAKE_OSX_SYSROOT=#{shell_output("xcrun --sdk macosx --show-sdk-path").chomp}
      -DCLANG_DEFAULT_OBJCOPY=/usr/local/opt/binutils/x86_64-apple-darwin#{shell_output("uname -r").chomp}/bin/objcopy
      -DCMAKE_OBJCOPY=/usr/local/opt/binutils/x86_64-apple-darwin#{shell_output("uname -r").chomp}/bin/objcopy
      -DCMAKE_OBJDUMP=/usr/local/opt/binutils/x86_64-apple-darwin#{shell_output("uname -r").chomp}/bin/objdump
